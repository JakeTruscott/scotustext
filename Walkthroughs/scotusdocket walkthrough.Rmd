...
`docket_search`serves as an automated tool for collecting, parsing, and cleaning information from docketed cases, petitions, and motions to the United States Supreme Court. Implementation of `docket_search` requires providing a docket number corresponding to the petition (or application/motion) of interest as a character object (`docket_id`). The function call will also accept a list of docket numbers, as well as an object containing a list of comma-separated character vectors. For more information on the Supreme Court docket, *see*   [supremecourt.gov](https://www.supremecourt.gov/docket/docket.aspx).

The process draws on 4 (potential) layers of iteration:   
1. `call_docket_data` retrieves an archived dataset on the primary `scotustext` [Github repository](https://github.com/JakeTruscott/scotustext.git). Any entries for `docket_id` found in the archived data will automatically be retrieved.    
**Note**: The dataset retains records of docketed petitions, motions, and applications filed between the 2001 and 2022 docketing years (as of June 2023), which will be periodically updated in the future.     
2. `process_docket_id` converts character entries of `docket_id` not retrieved from `call_docket_data` to URLs reflective of their location on [supremecourt.gov](https://www.supremecourt.gov/docket/docket.aspx).     
3.  For those values of `docket_id` processed using `process_docket_id`, `docket_search` will draw on `docket_call` and `process_url` to iteratively call to the docket and subsequently retrieve, clean, and parse the relevant data of interest.     
4. For those that values of `docket_id` that fail to retrieve data for whatever reason, `docket_search` will make two subsequent attempts to process -- which includes a full retry of the docket retrieval protocol, as well as trying a final time by altering the `https://` structure of the url to assess whether the `docket_id` was inadvertently assigned the incorrect .html or .htm structure to locate its storage location on [supremecourt.gov](https://www.supremecourt.gov/docket/docket.aspx).    
**Note**: Do not be alarmed by repeated messages in the **R** console that `docket_search` failed to retrieve information from specific docket numbers. As noted above, it will make several attempts to retrieve docket numbers of interest. Further, owing to the Court's transition from (.htm) to (.html) docket sheets with the incorporation of electronic filing during the 2016-2017 filing years, do not be alarmed if the first pass at dockets filed in the 2016 filing year (e.g., "16-1213") fail to retrieve information. Each docket filed in 2016 is assigned an (.htm) structure, though there is an indeterminate point where 2016 filings transition to (.html). There are special exception handling protocols to manage those filed in 2016 built into `docket_search` that will help to mitigate these issues on the second retrieval attempt. 
...
- `docket_number`: The docket number corresponding with the search query. 
...
- `docketed`: Date the petition (or application/motion) was docketed with the Court. 
...
- `number_of_opinions`: Integer indicating the total number of opinions filed (**Majority Opinion** plus any **Concurrences** and **Dissents**).
...
## `docket_search` **Example Walkthrough**

Here's an example of how to use the `docket_search` function using *Dobbs v. Jackson Women's Health Clinic* (2022), one of the largest docket sheets hosted by the Supreme Court. Each call to `docket_search` will initialize a search to the archived data on the primary `scotustext` [Github repository](https://github.com/JakeTruscott/scotustext.git), denote the number of retrieved values for `docket_id`, and finally indicate the number that need to be processed. Upon completion, `docket_search` will return a completion report along with summary statistics of a retrieved data. 
...
**Note**: Variables where multiple elements are returned (*e.g., multiple amici curiae filers*) are stored as flattended lists to allow for exporting to (.csv) and (.xlsx). However, these can be converted back to their original structure using the function below, which can also be called from the `scotustext` package: 
